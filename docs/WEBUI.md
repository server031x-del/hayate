...
生成画面は「プロンプト → 画像 → 長さ・サイズ → プロファイル」の順に並び、
生成ボタンと設定サマリーは画面下部に常に表示されます。選択中のプロファイルに
必要なモデルが未準備の場合、ボタンは無効になり理由を表示します。どの
プロファイルも使えないときは画面上部に「モデルの準備が必要です」と表示し、
設定画面へ移動できます。ヘッダーのエンジン表示は「生成可能 / 生成中 /
モデル未準備 / 設定を確認 / 接続が切れました」をモデル検証結果から判定します。

- **下書きの自動保存**: プロンプト、長さ、解像度、Seed、プロファイル、画像の
  選択はこのブラウザーの localStorage に保存され、再読み込み後も復元されます。
  サーバーやほかの端末には送信しません。
- **Seed**: 詳細設定の「毎回ランダム」をオンにすると `seed: null` を送信し、
  サーバーが抽選します。実際に使われた Seed は履歴に記録され、追加時に通知します。
- **画像**: Ctrl+V で開始画像（開始画像があれば終了画像）へ貼り付けできます。
  開始画像と出力の縦横比が異なる場合は「解像度を画像に合わせる」で近いプリセット
  または32の倍数のカスタムサイズへ変更できます。T2VA専用プロファイルでは画像入力を
  無効化します。FastH3 INT8 ではタスクを Auto とし、画像の有無から判定します。
- **キュー**: 実行中・待機中と最近のWebUIジョブを分けて表示します。待機中の
  ジョブは個別に取り消し、実行中のジョブは途中保存または中止できます。
- **ライブラリ**: 検索・状態絞り込み・並び替え（新しい順 / 古い順 / 生成時間 /
  解像度）に対応します。サムネイルは画面に入ったときだけ読み込み、マウスを重ねると
  プレビュー再生します。表示用のタイトルからは H3 の構造化フィールド名を省きます。
- **詳細ダイアログ**: 動画の再生・ダウンロード、プロンプトのコピー、解像度・Seed・
  ステップ・VRAMピーク・GPU などの実行情報、実行ログ（末尾400行、失敗時は自動で
  展開）、「この設定を再利用」、削除を1か所で操作できます。
- **設定**: 未保存の変更があると保存ボタンと表示で知らせ、ページを離れる前に
  確認します。ネットワーク表示は実際の待ち受け（LAN公開 / ローカルのみ）を
  `/api/bootstrap` の `network` から表示します。
- **エラー表示**: 生成前チェックの問題点や入力検証エラーを項目ごとに表示します。
- **ショートカット**: Ctrl+Enter で生成、Alt+1〜4 で画面切り替え、ライブラリで `/`
  を押すと検索欄へ移動します。ブラウザーの戻る・進むで画面を移動できます。

`tests/webui_availability.cjs` は DOM を使わずに画面制御ロジックを検証します。
Node.js がある環境では `pytest` からも実行されます。
