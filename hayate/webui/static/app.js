...
function renderModelSetup(payload) {
  ...
  const configurationNotes = {
    a100: `A100など40/80GB GPU向け。8-bit INT8 ConvRot DiTを使い、32B TEXTをストリーミングして40GBに収めます。「A100 高速・高画質」は生成後も常駐ワーカーがDiTをGPUに保持するため、待機中もVRAMを使います。${assets.length}ファイル · 合計 ${bytes(total)}（未準備 ${bytes(remaining)}）。取得後に「A100構成を適用」を押してから、「A100 高速・高画質」または「A100 品質基準」を選択してください。80GBではTEXTを常駐に切り替えられます。`,
